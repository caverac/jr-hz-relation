...
:mod:`experiments.form_factor` computes ``F(J_z, k) = <e^{-k|z|}>`` by a direct
...
as the sheet of :mod:`experiments.sheet` (with ``G = sigma_z = z0 = 1``), but its
...
def form_factor_orbit(energy: float, alpha: float, n_periods: int = 30, samples_per_period: int = 600) -> float:
    ...
    periods. This reproduces :func:`experiments.form_factor.vertical_form_factor`
